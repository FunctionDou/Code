...
void build_request(const char *url)
{
	...
	if (proxyhost == NULL)
	{
		...
		{
			// 如果 str 中的字符都没有在 accept 中出现，那么将返回 atr 的长度；检索的字符是区分大小写的.
			// 将 :// 后面的字符到 第一个 / 结束的 地址全部复制到 host 中.
			strncpy(host, url + i, strcspn(url + i, "/"));
		}
		// printf("Host=%s\n",host);
		// 相当于将 GET 与 www.biying.com(host) 结合起来
...
nexttry:
	while (1)
	{
		...
		if (force == 0)
		{
			...
			{
				if (timerexpired)
					break; //这个判断是 假如子进程已经执行了 alarm_handler 函数之后, timerexpired 为 1,那么就不要再重复执行下面的逻辑了
